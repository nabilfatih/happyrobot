...
Basic auth, server-only secrets, Effect Schema validation, Confect typed errors,
fixed-window rate limiting, hardening headers, and Docker deployment.
...
- Convex env stores `FMCSA_WEB_KEY`; Railway env stores HappyRobot/dashboard
  secrets and the shared `CONVEX_BACKEND_KEY`.
